...
class EWayPaymentController extends Controller implements NestedController {
	...
	public function __construct(Controller $parentController, SS_HTTPRequest $request, EWayPayment $payment) {
		parent::__construct();
	...
	public function handle_response($request) {
		...
		if ($response) {
			...
			if ($response->TrxnStatus == 'true') {
				...
				$this->payment->TansactionNumber = (string) $response->TrxnNumber;
	...
	protected function constructPurchaseURL() {
		...
		$params = array(
			...
			'Amount' => number_format($payment->Amount, 2), //2 decimal points required
			...
			'MerchantInvoice' => $payment->Invoice,
			'ModifiableCustomerDetails' => $config->EWay_CustomersCanModifyDetails
		);
		
		if ($config->EWay_CompanyLogo() && Director::protocol() == 'https://') {
			$params['CompanyLogo'] = $config->EWay_CompanyLogo()->getAbsoluteURL();
		}
		if ($config->EWay_PageBanner() && Director::protocol() == 'https://') {
